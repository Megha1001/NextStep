...
```
//TC : O(N) [every element is pushed and popped once]
 ...
 */
 ...
 134. Subsets
```
//TC : O(n*2^n) : 2^n subset and O(N) for copy res.add(new ArrayList<>(path));
//SC : O(N)
class Solution {
...
151. K Closest Points to Origin
```
//TC : O(nLogK)
//SC : O(k)
class Solution {
...
157. Find Median from Data Stream
```
class MedianFinder {
...
class Solution {
    public ListNode mergeKLists(ListNode[] lists) {
        ...
        return partitionedAndMerge(0, n - 1, lists);
    }

    private ListNode partitionedAndMerge(int s, int e, ListNode[] lists){
        if(s == e){
            return lists[s];
        }
        ...
        ListNode l1 = partitionedAndMerge(s, m, lists);
        ListNode l2 = partitionedAndMerge(m+1, e, lists);

        return mergedTwoList(l1, l2);
    }

    private ListNode mergedTwoList(ListNode l1, ListNode l2){
        ListNode dummy = new ListNode(0);
        ListNode head = dummy;
...
163. Median of Two Sorted Arrays
```
//TC : O(log(min(m, n)))
//SC : O(1)
class Solution {
...
178. Partition Equal Subset Sum
```
class Solution {
...
class Solution {
    ...
    public int findTargetSumWays(int[] nums, int target) {
        /*
        For every number we can choose either 
         +nums[i]
         -nums[i]
        */
        //Approach - 1 Memoization
...
184. Longest Palindromic Substring
```
//TC : O(N*N)
//SC : O(1)
class Solution {
...
185. Regular Expression Matching
```
//TC : O(2^(max(m, n)))
//SC : O(max(m,n))
class Solution {
...
class Solution {
    public int canCompleteCircuit(int[] gas, int[] cost) {
        ...
        if(totalKarch > totalKamai){
            return -1;
        }

        //now ans is guranteed
        int total = 0;
        int res = 0;
...
206. Insert Delete GetRandom O(1)
```
class RandomizedSet {
