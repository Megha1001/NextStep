...
35. String Matching in an Array
```
//TC : O(nLogN + N^2 * L)
//SC : O(1)
class Solution {
...
42.  Minimum Time to Make Rope Colorful
```
class Solution {
    public int minCost(String colors, int[] neededTime) {
        ...
        for(int i = 1; i < colors.length(); i++){
            if(colors.charAt(i) == colors.charAt(i-1)){
                ...
                maxTime = Math.max(maxTime, neededTime[i]); //retain maxTime one
            }else{
                //start next block
...
45. Maximum Bags With Full Capacity of Rocks
```
class Solution {
...
49. Minimum Replacements to Sort the Array
```
class Solution {
...
50. Minimum Number of Taps to Open to Water a Garden
```
class Solution {
